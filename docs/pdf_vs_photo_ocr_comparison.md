...
| 추가 비교 후보 | Android ML Kit OCR raw text를 같은 Quality Gate로 평가 |
| 결론 | MVP는 PDF/HWP/text 우선, 사진 OCR은 탑재형 image 입력 adapter 후보 |
...
태수님 파트는 HWP/PDF/text를 `clean_text(str)`로 변환하는 단계까지 담당한다. model input builder와 OCR quality gate는 이 변환 로직을 대체하지 않는다. 사진/image 입력 경로에서는 OCR 결과의 품질을 평가하고, 자동 확정이 어려운 경우 `review_text.txt`와 "원문 확인 필요" 판단 근거를 남긴다.

팀 메인 백엔드에는 OCR/image 입력을 붙일 자리가 마련되어 있다는 전제를 둔다. 따라서 이 실험의 질문은 "OCR을 할지 말지"가 아니라, "어떤 정확도와 품질 기준을 만족할 때 공통 파이프라인으로 넘길 수 있는지"다. 현재 OCR Lab은 사진 입력을 `clean_text`로 바로 확정하는 기능이 아니라, Quality Gate와 Field Recovery를 거쳐 안전한 연결 조건을 검증하는 image intake adapter 실험이다.

추가 비교 후보로 Android ML Kit OCR raw text도 같은 Quality Gate에 넣어 평가한다. 기존 비교 대상은 PDF 텍스트 변환과 Tesseract 기반 OCR Lab이었지만, Android OCR 결과 txt를 같은 기준으로 평가하면 PDF 변환 / Tesseract OCR / Android OCR을 한 테이블에서 비교할 수 있다. 단, 이 비교는 MVP 연결 결정이 아니라 사진 입력 확장 가능성과 한계를 보기 위한 근거다.
...
| 권장 사용 위치 | MVP 기본 입력 경로 | 탑재형 image 입력 adapter 후보 |

## 5.1 Android ML Kit OCR raw text Quality Gate 결과

Android ML Kit OCR 결과 `.txt` 4개를 `--text` 입력으로 Quality Gate에 넣었다. raw text에는 OCR Lab의 table/cell 구조 정보가 없으므로 `table.score`는 모두 `0.0`이며, `text_quality`와 `pattern` 점수 중심으로 해석한다.

| 파일 | 입력 조건 | auto_pass | overall_score | text_quality | pattern | table | dates | times | phones | urls | amounts |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| android_mlkit_113327.txt | PDF 1페이지 | false | 0.8184 | 0.7071 | 1.0000 | 0.0 | 4 | 0 | 1 | 1 | 0 |
| android_mlkit_112127.txt | 카메라 정면 촬영 | false | 0.8146 | 0.7009 | 1.0000 | 0.0 | 3 | 1 | 1 | 2 | 0 |
| android_mlkit_112155.txt | 카메라 기울임 촬영 | false | 0.8140 | 0.7000 | 1.0000 | 0.0 | 2 | 0 | 1 | 2 | 0 |
| android_mlkit_112228.txt | 카메라 구겨짐 촬영 | false | 0.8162 | 0.7035 | 1.0000 | 0.0 | 2 | 1 | 1 | 2 | 0 |

네 결과 모두 `auto_pass=false`다. 조정 후 공통 실패 이유는 `overall_score`가 기준값 0.90보다 낮다는 점으로 정리되었다. Android raw text에는 표 crop/cell 구조 정보가 없기 때문에 `scoring_mode: raw_text`로 평가하고, table 가중치는 0으로 둔다. 따라서 table 구조 부재가 직접 감점되지는 않는다. 정상 URL/전화번호가 함께 추출된 경우에는 일부 깨진 URL/전화번호 의심 후보를 hard fail 사유가 아니라 점검용 warning으로만 남긴다. Android OCR은 Tesseract OCR Lab과 달리 표 구조 점수는 없지만, 문장 단위 raw text 품질과 핵심 패턴 보존 정도를 같은 기준으로 볼 수 있다.
...
사진 OCR은 실제 사용자가 종이 안내문을 촬영해 올리는 상황을 위한 확장 후보로 보는 것이 맞다. 가능성은 있지만 전체 원문을 자동 확정하기에는 품질 변동이 크다. 현재 SchoolBridge 메인 흐름은 HWP/PDF/TXT 파일 업로드 기반의 구조화 파이프라인이고, OCR은 그 앞단에 붙는 image intake adapter로 두는 것이 적절하다.

두 방식 모두 결과를 그대로 정답으로 쓰면 안 된다. PDF 변환 결과도 중복, 줄바꿈, 숫자 섞임이 있고, 사진 OCR 결과는 회전, 흐림, 셀 복원, 날짜 오탐 문제가 있다.

날짜, URL, 전화번호 같은 핵심 필드는 모델이 추측하게 두기보다 정규식/후처리로 먼저 추출하고 보호하는 것이 맞다. 신뢰도가 낮거나 OCR 흔들림이 있는 필드는 자동 확정하지 않고 "원문 확인 필요"로 표시하는 UX가 필요하다.
...
금요일 오프라인 조립 시연에서는 이 기준을 그대로 사용한다. OCR 결과를 "이미 완성된 입력"으로 보여주기보다, 사진 입력에서 텍스트 후보를 만들고 Quality Gate가 `auto_pass=false`와 실패 이유를 남기는 흐름을 보여주는 쪽이 안전하다. 이 정도 정확도는 보조 입력/접근성 경로로는 의미가 있지만, 사용자 확인 없는 완전 자동 경로로는 아직 부족하다.
...
- 백엔드 OCR/image 입력 자리와 연결할 때의 최소 계약 정의
  - 입력: image 또는 Android ML Kit raw text
  - 출력: `clean_text_candidate`, `key_patterns`, `quality_report`, `review_required`
  - 자동 통과 조건: F1 0.9 이상, CER 0.1 이하급, 핵심 패턴 누락 없음
  - 기준 미달 조건: 공통 파이프라인으로 바로 넘기지 않고 원문 확인 UX로 전환
...
하지만 사진 OCR은 영양성분 숫자 오탐, 메뉴 셀 복원 부족, 흐린 사진 품질 저하가 확인되었기 때문에 전체 원문을 자동 확정하는 입력으로 바로 쓰기에는 아직 부족하다. 최종 목표는 F1 0.9 이상, CER 0.1 이하 수준의 자동 처리지만, 현재 실험 결론은 사진 입력을 image intake adapter로 두고 `review_required` 또는 "원문 확인 필요" UX를 전제로 해야 한다는 것이다.

두 방식 모두 결과를 그대로 정답으로 쓰면 안 된다. 윤정님 모델에 넣기 전에는 줄바꿈, 중복, 숫자 오탐, 표 구조 깨짐을 정리해야 한다. 현재 기준으로는 PDF/HWP/text 변환 결과를 우선 `clean_text` 경로로 유지하고, 사진 OCR은 백엔드 OCR 자리와 연결 가능한 Quality Gate / Field Recovery 계약을 검증하는 방향이 좋다.
