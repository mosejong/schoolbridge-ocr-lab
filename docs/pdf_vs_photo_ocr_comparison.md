...
추가 비교 후보로 Android ML Kit OCR raw text도 같은 Quality Gate에 넣어 평가한다. 기존 비교 대상은 PDF 텍스트 변환과 Tesseract 기반 OCR Lab이었지만, Android OCR 결과 txt를 같은 기준으로 평가하면 PDF 변환 / Tesseract OCR / Android OCR을 한 테이블에서 비교할 수 있다.
